...
const MainPowerCategoryList = () => {
  ...
  let addcategorydata = () => {
    swal({
      title: "Main Power Category added Successfully",
      text: "Main Power Category inserted successfully",
      icon: "success",
      buttons: true,
    });
  };
  const categorydatahandle = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/admin_add_cate_main_power`,
        formData,
        options
      )
      .then((res) => {
        toast.success(res?.data?.msg || "Main Power Category Add Successfully");
        addcategorydata();
        getbannerlist();
      })
      .catch((error) => {
  ...
  let deletesubcategory = (item) => {
    swal({
      ...
      if (willDelete) {
        ...
        deletebannerimage();
        swal("Poof! Your Main Power category  has been deleted!", {
          icon: "success",
        });
      } else {
        swal("Your Main Power category is safe!");
      }
    });
  };
...
export default MainPowerCategoryList;

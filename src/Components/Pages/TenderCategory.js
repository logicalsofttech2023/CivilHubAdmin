...
const TenderCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/all_tender_category`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_tender_category`,
        formData,
        {
          headers: {
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/tender_cate_delete/${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {

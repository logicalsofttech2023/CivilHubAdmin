...
const ServiceSubCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/service_sub_cat_list`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_service_sub_category`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/service_sub_cat_delete/${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
  ...
  const handleEditCategory = (id) => {
    navigate("/updateServiceSubCategory", {
      state: { categoryId: id },
    });
  };
  ...
  const renderCategoryRow = (category, index) => {
    ...
    return (
      <tr key={category._id}>
        ...
        <td>{category?.servicecate_id?.title || "N/A"}</td>

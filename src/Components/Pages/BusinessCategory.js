...
const BusinessCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/all_bussiness_category`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_admin_bussiness_cate`,
        formData,
        {
          headers: {
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/delete_bussiness_cate/${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
  ...
  const handleEditCategory = (id) => {
    navigate("/updateBusinessCategory", {
      state: { categoryId: id },
    });
  };

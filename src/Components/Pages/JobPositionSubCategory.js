...
const JobPositionSubCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/getAllJobPositionSubCategory`, {
  ...
  let getServiceCategoryList = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}admin/api/GetAllJobPositionCategory`,
        options
      )
      .then((res) => {
  ...
  const handleAddCategory = (e) => {
    ...
    formData.append("jobPositionCategoryId", selectedCategory);
    if (categoryImage) {
      formData.append("image", categoryImage);
    }
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/CreateJobPositionSubCategory `,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/DeleteJobPositionSubCategory?id=${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
  ...
  const handleEditCategory = (id) => {
    navigate("/updateJobPositionSubCategory", {
      state: { categoryId: id },
    });
  };
  ...
  const renderCategoryRow = (category, index) => {
    ...
    return (
      <tr key={category._id}>
        ...
        <td>{category?.name}</td>
        <td>{category?.jobPositionCategoryId?.name || "N/A"}</td>
...
export default JobPositionSubCategory;

...
const UpdateProductCategory = () => {
  ...
  const [fetching, setFetching] = useState(true); // For initial GET
  ...
  let getcategorydetails = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_API_KEY}admin/api/market_category_details/${category_id}`,
        options
      )
      .then((res) => {
  ...
  const editcetagory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/update_market_category`,
        formData,
        options
      )
      .then((res) => {
        toast.success(res?.data?.msg || "Product Category Updated");
        editdata();
        setTimeout(() => Navigate("/productCategory"), 2000);
      })
      .catch((error) => {

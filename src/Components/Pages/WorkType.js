...
const WorkType = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/get_work_cate`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_work_cate`,
        {
          title: categoryName,
        },
        {
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/delete_work_cate/${id}`,

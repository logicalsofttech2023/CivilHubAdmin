...
const ServiceProviderSubCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/all_service_provider_sub_cat`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_service_provider_sub_cat`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/service_provider_sub_cat_delete/${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
  ...
  const handleEditCategory = (id) => {
    navigate("/updateServiceProviderSubCategory", {
      state: { categoryId: id },
    });
  };
  ...
  const renderCategoryRow = (category, index) => {
    ...
    return (
      <tr key={category._id}>
        ...
        <td>{category?.serviceprovidercat_id?.title || "N/A"}</td>
            ...
            <div className="mb-3">
              <h2 className="h1 mb-0 d-flex gap-10">
                <img
                  ...
                  alt=""
                />
                Service Provider Sub Categories
              </h2>
            </div>
...
export default ServiceProviderSubCategory;

...
const TenderDepartment = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/get_department_cate`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/add_department_cate`,
        {
          title: categoryName,
        },
        {
  ...
  swal({
    ...
    if (confirm) {
      axios
        .post(
          `${process.env.REACT_APP_API_KEY}admin/api/delete_department_cat/${id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleEditCategory = (id) => {
    navigate("/updateTenderDepartment", {
      state: { categoryId: id },
    });
  };
...
export default TenderDepartment;

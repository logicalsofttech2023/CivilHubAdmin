...
const JobPositionCategory = () => {
  ...
  const fetchCategories = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API_KEY}admin/api/GetAllJobPositionCategory`, {
  ...
  const handleAddCategory = (e) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_KEY}admin/api/CreateJobPositionCategory`,
        formData,
        {
          headers: {
  ...
  const handleDeleteCategory = (id) => {
    swal({
      ...
      if (confirm) {
        axios
          .get(
            `${process.env.REACT_APP_API_KEY}admin/api/DeleteJobPositionCategory?id=${id}`,
            { headers: { Authorization: `Bearer ${token}` } }
          )
          .then(() => {
  ...
  const handleEditCategory = (id) => {
    navigate("/updateJobPositionCategory", {
      state: { categoryId: id },
    });
  };
